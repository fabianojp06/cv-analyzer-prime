      ...
      <DialogContent className="max-w-2xl bg-card border-border p-0 gap-0 overflow-hidden">
        ...
        <DialogHeader className="p-6 pb-4 border-b border-border">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-primary/20 text-primary flex items-center justify-center text-lg font-bold flex-shrink-0">
              {initials}
            </div>
            <div className="flex-1 min-w-0">
              <DialogTitle className="text-xl font-bold text-foreground">{candidate.name}</DialogTitle>
              <DialogDescription className="text-sm text-muted-foreground mt-0.5">{candidate.role}</DialogDescription>
            ...
            <div className="flex flex-col items-center gap-1">
              <RingProgress value={candidate.aiScore} size={64} strokeWidth={5} />
        ...
        <Tabs defaultValue="insights" className="flex-1">
          <TabsList className="w-full justify-start rounded-none border-b border-border bg-transparent px-6 h-auto py-0">
            <TabsTrigger value="insights" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none py-3 text-sm">
              <Sparkles className="h-4 w-4 mr-1.5" /> AI Insights
            </TabsTrigger>
            <TabsTrigger value="resume" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none py-3 text-sm">
              <Briefcase className="h-4 w-4 mr-1.5" /> Currículo
            </TabsTrigger>
          </TabsList>
            ...
            <div className="rounded-xl border border-primary/30 bg-primary/5 p-5 relative overflow-hidden">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">{candidate.aiJustification}</p>
                </div>
              </div>
            </div>
            <div className="mt-4">
          ...
          <TabsContent value="resume" className="p-6 mt-0 space-y-6">
            <div>
        ...
        {/* Footer */}
        <DialogFooter className="p-4 border-t border-border bg-card/80">
          <div className="flex w-full gap-2 sm:justify-end">
            <Button variant="outline" size="sm" className="gap-1.5">
              <Download className="h-3.5 w-3.5" /> Baixar PDF
            </Button>
            <Button variant="destructive" size="sm" className="gap-1.5 bg-score-low hover:bg-score-low/90">
              <XCircle className="h-3.5 w-3.5" /> Rejeitar
            </Button>
            <Button size="sm" className="gap-1.5 bg-primary hover:bg-primary/90">
              <CheckCircle className="h-3.5 w-3.5" /> Aprovar para Entrevista
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
